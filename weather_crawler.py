...
CWA_PROXY = {"http": "http://127.0.0.1:7897", "https": "http://127.0.0.1:7897"}
...
def load_all_stations(data_dir=None):
    ...
    result = {}
    for name in ["nmc", "jma", "kma", "cwa", "synop"]:
...
class KMASource:
    ...
    def fetch_single(self, station_code):  # 去掉 session
        info = self.stations.get(station_code)
        if not info:
            return station_code, station_code, '', {}, ''
        name = info['name']
        ...
        try:
            ...
            if resp.status_code != 200:
                return station_code, name, region, {}, ''
            soup = BeautifulSoup(resp.content, 'html.parser')
            # 找到所有数据行（具有 bgcolor 属性的 tr）
            rows = soup.find_all('tr', bgcolor=True)
            for row in rows:
                cells = row.find_all('td')
                if len(cells) < 17:
                    continue
                # 温度列（第 9 列，索引 8）有效时才是数据行
                temp_val = cells[8].text.strip()
                if temp_val == '.' or temp_val == '':
                    continue

                # 按表头顺序提取各要素
                # 索引 3: 강수60M (1小时降水)
                precip_60m = cells[3].text.strip()
                # 索引 13: 풍향10M 방향 (10米风向英文)
                wind_dir = cells[13].text.strip()
                # 索引 14: 풍속10M (10米风速)
                wind_speed = cells[14].text.strip()
                # 索引 15: 습도%
                humidity = cells[15].text.strip()
                # 索引 16: 해면기압hPa (海平面气压)
                pressure = cells[16].text.strip()
                # 索引 0: 시간
                ...
                weather_data = {
                    ...
                    '地面气压': clean(pressure),  # 暂用海平面气压，后续可据站点海拔修正
                ...
                try:
                    ...
                    if dt_kst > now_kst:          # 若解析时间超过当前时间，则修正为前一天
...
class CWASource:
    ...
    def _fetch_main(self, session, api_station_id):
        ...
        try:
            resp = session.get(self.MAIN_API_URL, params=params, timeout=REQUEST_TIMEOUT, proxies=CWA_PROXY)
            if resp.status_code != 200:
                return None
            data = resp.json()
    ...
    def _fetch_rain(self, session, api_station_id):
        ...
        try:
            resp = session.get(self.RAIN_API_URL, params=params, timeout=REQUEST_TIMEOUT, proxies=CWA_PROXY)
            if resp.status_code != 200:
                return 'N/A'
            data = resp.json()
            stations = data.get('records', {}).get('Station', [])
...
class SynopSource:
    ...
    def fetch_single(self, station_code):
        ...
        try:
            ...
            for _ in range(5):
                resp = session.get(self._make_url(station_code, dt), headers=headers, timeout=REQUEST_TIMEOUT)
